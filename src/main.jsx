...
import aronageLogoLight from './assets/brand/aronage-logo-light.svg';
...
function BrandLogo({light=false}){
  return <span className={`brand-logo ${light?'light':''}`} style={{'--brand-logo':`url(${light?aronageLogoLight:aronageLogo})`}} role="img" aria-label="آروناژ انرژی"/>
}
...
function StoreHeader(){const[open,setOpen]=useState(false);return <header className="store-header scrolled"><button className="brand brand-with-logo" onClick={()=>navigateTo('/')} aria-label="بازگشت به صفحه اصلی آروناژ"><BrandLogo light/></button><nav className={open?'open':''} aria-label="منوی فروشگاه"><button onClick={()=>navigateTo('/')}>صفحه اصلی</button><button onClick={()=>navigateTo('/shop')}>محصولات</button><a href={phoneHref}>مشاوره خرید</a></nav><div className="head-actions"><a className="btn" href={phoneHref}>تماس بگیرید <ArrowUpLeft size={18}/></a><button className="menu-btn" onClick={()=>setOpen(!open)} aria-label="منوی فروشگاه" aria-expanded={open}>{open?<X/>:<Menu/>}</button></div></header>}
...
function App(){
  ...
  return <div className="site-shell">
    <header className={scrolled?'scrolled':''}>
      <a className="brand brand-with-logo" href="#top" aria-label="آروناژ انرژی"><BrandLogo light/></a>
    ...
    <footer id="contact" data-robot-scene="contact">
      ...
      <div className="footer-contact-strip"><div><MapPin/><span><small>دفتر مرکزی آروناژ</small><b>تهران، احمدآباد مستوفی، خیابان احسانی‌راد، سازمان پژوهش‌های علمی و صنعتی ایران، پارک علم و فناوری</b></span></div><div className="footer-phone-list"><a href={phoneHref}>۰۹۱۲۶۱۹۳۹۸۴</a><a href="tel:+989339609030">۰۹۳۳۹۶۰۹۰۳۰</a><a href="tel:+989126910915">۰۹۱۲۶۹۱۰۹۱۵</a></div></div>
